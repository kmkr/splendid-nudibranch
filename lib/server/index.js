...
var _viewDataService = require('./view-data-service');

var viewDataService = _interopRequireWildcard(_viewDataService);

var _photoDataConversion = require('./photos/photo-data-conversion');
...
function photoIndex(res, _ref) {
  ...
  return Promise.all([viewDataService.getPhotoData(), viewDataService.getKeywords()]).then(function (_ref2) {
    ...
    var photos = photoData.photos.map(function (p) {
      return (0, _photoDataConversion.serverToClient)(p, photoData.base);
    });
    return res.render('index', {
      description: _constants.description,
      photos: JSON.stringify(photos),
      ogTags: (0, _ogTags2.default)(photos, { selectedPhotoKey: photoKey, year: year, location: location }),
